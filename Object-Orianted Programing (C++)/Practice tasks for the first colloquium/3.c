...
void pecati(ZabavenPark *Zp, int n){
    for(int i=0; i<n; i++){
        ...
        for(int j=0; j<Zp[i].brojElementi; j++){
            printf("%s %d %.2f\n",Zp[i].niza[j].ime,Zp[i].niza[j].vremeMin,Zp[i].niza[j].cena);
        }
    }
}
...
void najdobar_park(ZabavenPark *zp, int n){
    ...
    int indexR;
    int maxMinutes;
    int maxMinutesSave = 0;
    int i,j,r;
    for(int i=0; i<n; i++){
        max = 0;
        for(int j=0; j<zp[i].brojElementi; j++){
            if(zp[i].niza[j].popust == 1)
            {
                max++;
            }
            if(maxSave < max){
            ...
            if(maxSave == max){
                maxMinutes = 0;
                for(int r = 0; r < zp[i].brojElementi; r++){
                    maxMinutes = maxMinutes + zp[i].niza[r].vremeMin;
                }
                if(maxMinutesSave < maxMinutes){
                    maxMinutesSave = maxMinutes;
                    index = i;
                }
            }
        }
...
int main() {
    ...
    for (i = 0; i < n; i++){
        ...
        //broj na filmovi
        scanf("%d", &ff[i].brojElementi);

        for (j = 0; j < ff[i].brojElementi; j++){
            scanf("%s", ff[i].niza[j].ime);  			/* Ime na filmot */
            scanf("%d", &ff[i].niza[j].vremeMin); 		/* Vremetraenje   */
            scanf("%f", &ff[i].niza[j].cena); 	/* Cena */
            scanf("%d", &ff[i].niza[j].popust);	/* Popust */
