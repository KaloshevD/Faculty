...
public:
    ...
    virtual int getBodovi(){
        ...
        return (sum*100)/brojOcenki;///tuka smeniv za vtor test case
    }
    virtual void pecati(){
        cout<<indeks;
    }
...
private:
    int brojCasovi;
public:
    Demonstrator(int indeks = 0,int *ocenki = 0, int brojOcenki = 0, const char *imeIPrezime = "",Kurs *kursevi= 0,int brojKursevi = 0, int brojCasovi = 0)
...
Student& vratiNajdobroRangiran(Student ** studenti, int n ){
    ...
    for(int i=0;i<n;i++){
        try{
            if (studenti[i]->getBodovi() > tmp->getBodovi()) {
                tmp=studenti[i];
            }
        }
