...
public class ListaOdListi {
    public static long findMagicNumber(DLL<DLL<Integer>> list) {
        long num = 1;
        DLLNode<DLL<Integer>> nodeInsideNodeList = list.getFirst();
        DLLNode<Integer> tempnum;
        int sumOfNodes = 0;
        for (int i = 0; i < list.length(); i++) {
            tempnum = nodeInsideNodeList.element.getFirst();
            sumOfNodes = 0;
            while (tempnum != null) {
                sumOfNodes += tempnum.element;
                tempnum = tempnum.succ;
            }
            nodeInsideNodeList = nodeInsideNodeList.succ;
            num *= sumOfNodes;
        }
        return num;
